...
term.overlap <- function(coefs, cell.line, ontology = "BP|CC|MF|KEGG|WP") {
  ...
  if (ontology %in% c("BP", "CC", "MF")) {
    for (l in cell.line) {
      files <- list.files(paste0("figures/cross-analysis/gsego_results/gsego_", ontology, "_results/"), 
                          full.names = TRUE,
                          pattern = "^simplified")
    ...
    for (l in cell.line) {
      files <- list.files(paste0("figures/cross-analysis/gsekegg_results/"), 
                          full.names = TRUE,
                          pattern = ".tsv")
    ...
    for (l in cell.line) {
      files <- list.files(paste0("figures/cross-analysis/gseWP_results/"), 
                          full.names = TRUE,
                          pattern = ".tsv")
  ...
  for (i in curr.comp) {
    ...
    ggsave(
      ...
      plot = temp,
      path = paste0("figures/cross-analysis/overlap/terms/",ontology),
      device = "png",
      units = "px", 
      dpi = 100,
      width = 500,
