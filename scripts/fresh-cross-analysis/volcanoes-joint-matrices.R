...
## loop -----------------------------------------------------------------------
for (i in coefs) {
  if (dir.exists("figures/cross-analysis/volcanoes/") == FALSE) {
    dir.create("figures/cross-analysis/volcanoes/",showWarnings = TRUE,recursive = TRUE)
  }
  if (dir.exists("data/cross-analysis/comparison_stats/") == FALSE) {
    dir.create("data/cross-analysis/comparison_stats/",showWarnings = TRUE,recursive = TRUE)
  }
  ...
  write.table(file = paste0("data/cross-analysis/comparison_stats/", names(coefs[i]), ".tsv"), 
              x = volc_data, 
              quote = FALSE, 
              sep = "\t",
              row.names = FALSE)
  ...
  ggsave(filename = paste0("volcano", names(coefs[i]), ".png"),
         path = "figures/cross-analysis/volcanoes/",
         device = "png",
         width = 640,
         height = 660,
         units = "px",
