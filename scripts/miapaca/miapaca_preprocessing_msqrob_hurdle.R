...
library(tidyverse)
fc_msqrob <- read.delim("data/miapaca/a_Hurdle_Msqrob.tsv")
...
# change the sign to '-' so the reference is on the left
...
# EDA---------------------------------------------------------------------------
p <- pivot_longer(int_mat, 1:15, names_to = "Sample", values_to = "Intensity") %>%
  ggplot(aes(x=Sample, y=Intensity))+
  geom_violin()+
  geom_boxplot(width=.3)+
  ggpubr::rotate_x_text(angle=45)
ggsave(plot = p, filename="boxplots.png", device = "png",path = "figures/miapaca", dpi = 100, units = "px", width=800)

p <- pivot_longer(qnorm, 1:15, names_to = "Sample", values_to = "Intensity") %>%
  ggplot(aes(x=Sample, y=Intensity))+
  geom_violin()+
  geom_boxplot(width=.3)+
  ggpubr::rotate_x_text(angle=45)
ggsave(plot = p, filename="boxplots-post-norm.png", device = "png",path = "figures/miapaca", dpi = 100, units = "px", width=800)
