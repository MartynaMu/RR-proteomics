...
library(tidyverse)
fc_msqrob <- read.delim("data/cfpac/a_Hurdle_Msqrob.tsv")
...
ggsave(plot = p, filename="boxplots.png", device = "png",path = "figures/cfpac", dpi = 100, units = "px", width=800)
...
p <- pivot_longer(qnorm, 1:15, names_to = "Sample", values_to = "Intensity") %>%
  ...
  geom_boxplot(width=.3)+
  ggpubr::rotate_x_text(angle=45)
ggsave(plot = p, filename="boxplots-post-norm.png", device = "png",path = "figures/cfpac", dpi = 100, units = "px", width=800)
