...
## loop -----------------------------------------------------------------------
for (i in coefs) {
  if (dir.exists("figures/cfpac/int_mat_fc/volcanoes/") == FALSE) {
    dir.create("figures/cfpac/int_mat_fc/volcanoes/",showWarnings = TRUE,recursive = TRUE)
  }
  if (dir.exists("data/cfpac/int_mat_fc/comparison_stats/") == FALSE) {
    dir.create("data/cfpac/int_mat_fc/comparison_stats/",showWarnings = TRUE,recursive = TRUE)
  }
  ...
  write.table(file = paste0("data/cfpac/int_mat_fc/comparison_stats/", names(coefs[i]), ".tsv"), 
              x = volc_data, 
              quote = FALSE, 
              sep = "\t",
              row.names = FALSE)
  ...
  ggsave(filename = paste0("volcano", names(coefs[i]), ".png"),
         path = "figures/cfpac/int_mat_fc/volcanoes/",
         device = "png",
         width = 640,
         height = 660,
         units = "px",
